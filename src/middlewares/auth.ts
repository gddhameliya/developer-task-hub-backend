...
const auth = ({ isTokenRequired = true, usersAllowed = [] }: AuthOptions) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    //* check if user role is allowed to access the route or not
    if (!usersAllowed.includes(user.role)) {
      return ApiResponse.FORBIDDEN({ res, message: "Access denied. You are not allowed to access this route." });
    }
